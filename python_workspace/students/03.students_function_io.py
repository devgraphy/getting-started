# ver3. 수강생 관리 시스템(파일 io를 통한 데이터 유지되는 프로그램)
import os.path # ???? 시스템에서 해당되는 path 작성하는데 도움이된다??? 뭔말이여
...
# 프로그램 종료 시 list students "students.dat" 파일
def save_list():
    save_file = open("students.dat","w")
    for num, student in enumerate(students):
        # 파이프라인을 기준으로 크기가 2인 배열에 담음. 
        save_file.write("{}번째 | {},{},{},{}\n".format(num, student["id"],student["name"],student["age"],student["major"]) )
    save_file.close()

# 프로그램 시작 시 "students.dat" 파일이 존재하고 정보가 있는 경우 list students 초기화
# 1번째 원소를 , 로 구분해서 읽어들인다.
def init_data_load():
    ...
    if fileExist:
        ...
        while True: # 토큰 자르기
            ...
            if len(data.split("|")) == 2:   # 원소 수로 valid 체크
            ...
            if not data : break # 데이터 없으면 빠져 나가기
        read_file.close()    
...
while True:
    ...
    if menu == '1':             
        ...
        students.clear()
        message_display("모든 수강자 정보가 삭제되었습니다.\n")
    elif menu == '0':
        save_list()
